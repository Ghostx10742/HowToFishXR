...
The output filename uses the independent version in `thunderstore/manifest.json` (currently `release/HowToFishXR-Thunderstore-v1.0.1.zip`). Its root contains the Thunderstore-specific `manifest.json`, `README.md`, `icon.png`, showcase GIFs in `images/`, and the mod's routed `BepInEx/plugins` and `BepInEx/patchers` folders. The manifest declares `BepInEx-BepInExPack-5.4.2305` as a dependency.
